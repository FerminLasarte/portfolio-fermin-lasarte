...
- **Los bloques largos son opcionales:** `context`, `build`, `decisions` y `result` se muestran **solo si el proyecto tiene ese texto** (`projects.<id>.context` y compañía). Los tienen DeporTurnos, TravelPic, Vault, chatbot-ai y Bookit (2026-09-22/23); los demás salen con el resumen, la placa, las tecnologías y los enlaces, y se llenan sin tocar código. Una clave que no está en ninguno de los dos idiomas no rompe el control de paridad de `scripts/check.mjs`. Cómo se recorre la página está en 7.13.
