...
- **Proyectos (`/proyectos`):** el título, una frase y las seis tarjetas de la home en vertical, en dos columnas desde tablet. Son **el mismo componente** que la home (`ProjectCard` con `panel={false}`, que apaga lo de la pista): una sola definición de cómo se resume un proyecto. Existe sobre todo para que las páginas de proyecto tengan madre y las migas de pan cierren (Inicio › Proyectos › TravelPic).
- **Proyecto (`/proyectos/<id>`):** el número y la vuelta a la lista, el título en Display, el problema y la solución, el estado, las plataformas y los años (que salen de Trayectoria cuando el proyecto también es una etapa: no se inventan los que no están), la placa si hay una imagen de verdad, los bloques largos, las tecnologías con los enlaces de afuera, el anterior y el siguiente en círculo, y "Volver al inicio", que vuelve a la tarjeta de ese proyecto y no al principio de la sección. La placa tipográfica no se muestra acá: es el nombre en grande y el `<h1>` ya es eso mismo; a lo ancho de la página quedaba una banda de color repitiendo el título. La placa, la meta y los enlaces son los mismos componentes que usa la tarjeta (`ProjectPlate`, `ProjectMeta`, `ProjectLinks`).
- **Los bloques largos son opcionales:** `context`, `build`, `decisions` y `result` se muestran **solo si el proyecto tiene ese texto** (`projects.<id>.context` y compañía). Desde el 2026-09-23 los tienen los seis proyectos; uno nuevo sale con el resumen, la placa, las tecnologías y los enlaces hasta que tenga los suyos, y se llena sin tocar código. Una clave que no está en ninguno de los dos idiomas no rompe el control de paridad de `scripts/check.mjs`. Cómo se recorre la página está en 7.13.
...
- **Entradas:** las de los paneles (8, "Entrada de cada panel"), con el mismo código. La lógica salió de `TrackController` a `lib/reveal.js` y la usan la pista (un grupo por panel) y `components/PageReveal.jsx` (un grupo por bloque marcado con `data-reveal`: la placa, cada capítulo, las tecnologías y el pie). Los títulos de los capítulos suben en su máscara, la placa se destapa desde abajo y el resto sube con un fundido, 60ms detrás del anterior. Si se llega con la cortina, lo que ya se ve entra cuando la cortina se va, como en la home. El encabezado tiene su propia entrada al cargar.
...
- **Sin soporte, sin JS o con reduce motion:** la placa quieta, la línea entera y el siguiente proyecto lleno. En colores forzados, la línea y los puntos van en `CanvasText` y el siguiente proyecto, lleno en el color de los enlaces.
