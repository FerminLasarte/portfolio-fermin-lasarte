...
export const PERSON = {
  ...
  city: "Buenos Aires",
  country: "Argentina",
  // Su zona horaria: la hora en vivo de la línea "Base" del hero (components/LocalTime.jsx).
...
// En qué se enfoca: la línea "Enfoque" del hero. Igual en los dos idiomas.
export const FOCUS = ["iOS", "Flutter", "Backend"];
...
// (/trabajos/<id>, el mismo en los dos idiomas), así que cambiarlo rompe una URL
...
// (si falta se usa `name`), `name` (el del índice, si cambia con el idioma), `problem`
...
//              línea de meta del índice y de la página (components/ProjectMeta.jsx).
...
//   media:     las pantallas del proyecto: en la vista previa del índice (docs/DISENO.md,
//              7.3) y en su página, al lado de los capítulos (7.13).
//              { type: "type" }: sin captura; el índice muestra el nombre en grande.
...
//              { type: "shot", image, width, height, gallery }: la captura vertical de la
//              app, en un teléfono. La imagen tiene que tener proporción de teléfono
//              (9:19,5; lo que sobra abajo se corta).
//              Las capturas necesitan su descripción en translations,
//              `projects.<id>.shotAlt`: en la página del proyecto son contenido, no
//              decoración. `gallery` (opcional) son más capturas, `{ key, image, width,
//              height }` (y `imageDark` en las ventanas), que solo muestra la página del
//              proyecto; su descripción va en `projects.<id>.shotAlt.<key>`. La página
//              muestra la captura n mientras se lee el capítulo n (la principal con
//              Contexto); los capítulos que sobran, la última.
//   links:     "appstore" | "playstore" | "repo" | "demo": las tiendas, el código y la
//              web. Van en cada fila del índice (docs/DISENO.md, 7.3) y al pie de la
//              página del proyecto, donde el primero es el botón relleno.
export const PROJECTS = [
  {
    ...
    status: "relaunch",
    platforms: ["ios", "android"],
    // Capturas del simulador (Fermin sigue con el soporte): Inicio en el índice, y en
    // la página, con el perfil de un fotógrafo y los medios de pago.
    media: {
      type: "shot",
  ...
  },
  {
    ...
    status: "live",
    platforms: ["ios", "android"],
    // Capturas de la app publicada (Fermin la vendió, así que no hay simulador), 828×1800,
    // sin la barra de estado. Reservar en el índice; en la página, el recorrido de una
    // reserva: los filtros, el complejo, sus horarios y la confirmación. Sin la foto de
    // la cuenta ni el perfil de otro jugador ("¿Con quién jugás?").
    media: {
      type: "shot",
      ...
      height: 1800,
      gallery: [
        { key: "filters", image: "/assets/deporturnos-filtros.webp", width: 828, height: 1800 },
        { key: "venue", image: "/assets/deporturnos-predio.webp", width: 828, height: 1800 },
        { key: "slots", image: "/assets/deporturnos-horarios.webp", width: 828, height: 1800 },
        { key: "confirm", image: "/assets/deporturnos-confirmar.webp", width: 828, height: 1800 },
      ],
    },
  ...
  },
  {
    ...
    // tema del sitio), sin la franja negra de arriba: 1600×1004. Estadísticas en el
    // índice; en la página, con Transacciones y Ahorros.
    media: {
      type: "window",
  ...
  },
  {
    ...
    status: "live",
    platforms: ["ios", "android"],
    // Capturas del simulador con datos de ejemplo: Inicio (quien saca turnos) en el
    // índice; en la página, con la elección del horario y la agenda del local, los dos
    // lados de la app.
    media: {
      type: "shot",
...
// de la etapa), `.title` (el rol), `.short` (la línea de la home) y `.desc`. `where` es
// igual en los dos idiomas. `end: null` es lo que sigue en curso: se muestra "2026–" y
// "en curso". El `id` es el del proyecto, así la etapa lleva a su página.
export const EXPERIENCE = [
  { id: "bookit", start: 2026, end: null, where: "Tandil", tags: ["Flutter", "Supabase", "Next.js"] },
  { id: "chatbot", start: 2026, end: null, where: "Argentina", tags: ["Python", "FastAPI", "Claude", "pgvector"] },
...
// El trabajo de una etapa de Trayectoria (tienen el mismo `id`).
const projectOf = (stage) => PROJECTS.find((p) => p.id === stage.id);

// Lo que sigue en curso, en el orden de EXPERIENCE: la línea "Ahora" del hero.
export const CURRENT_WORK = EXPERIENCE.filter((e) => e.end == null).map(projectOf);

// El último trabajo en producción con capturas de teléfono (el de la etapa más
// reciente): la tarjeta del hero. Si ninguno tiene capturas, el hero no la muestra.
export const LATEST_WORK = [...EXPERIENCE]
  .sort((a, b) => b.start - a.start)
  .map(projectOf)
  .find((p) => p.status === "live" && p.media.type === "shot");
...
// Cifras de la entrada de Trabajos. appsLive cuenta los trabajos en producción
// (`status: "live"`): las apps móviles, las de escritorio y los servicios como
// chatbot-ai.
export const STATS = {
  appsLive: PROJECTS.filter((p) => p.status === "live").length,
  // Lo que sigue en curso cuenta hasta el año actual. Con ISR, la página se regenera
  // una vez por día (app/[lang]/layout.js), así cambia sola al pasar de año.
  years:
    Math.max(...EXPERIENCE.map((e) => e.end ?? new Date().getFullYear())) -
    Math.min(...EXPERIENCE.map((e) => e.start)),
