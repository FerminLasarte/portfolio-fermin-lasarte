...
- **Los bloques largos son opcionales:** `context`, `build`, `decisions` y `result` se muestran **solo si el proyecto tiene ese texto** (`projects.<id>.context` y compañía). Hoy no lo tiene ninguno, así que las páginas salen con el resumen, la placa, las tecnologías y los enlaces, y se llenan sin tocar código. Una clave que no está en ninguno de los dos idiomas no rompe el control de paridad de `scripts/check.mjs`.
