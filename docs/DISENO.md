...
Pedido por Fermin el 2026-09-22, al escribir la de DeporTurnos: que la página, su recorrido y sus animaciones se sientan parte del portfolio. Por eso **no inventa movimientos nuevos**: usa los de la home, en vertical.
...
- **Por qué el movimiento va por JS y no por CSS.** La primera versión animaba la propiedad CSS `d` con `@keyframes`, dentro de un `@supports (d: path(...))`. Funciona en Chrome y en Firefox, pero **Safari no soporta esa propiedad en ninguna versión** (caniuse, hasta la 27.2), así que ahí el `@supports` daba falso, `.curtain` nunca salía de `display: none` y, como `Curtain.jsx` preguntaba lo mismo con `CSS.supports`, tampoco frenaba el clic: la cortina no se veía y el parpadeo del hero volvía tal cual. Lo encontró Fermin en producción el 2026-09-21. El atributo `d` sí se puede escribir en todos lados, así que ahora lo interpola `lib/curtain.js` (`mezclar` y `animar`) y queda un solo camino de código, sin `@supports`. En CSS quedan el nombre y la línea, que van con `translate`, `opacity` y `scale`.
