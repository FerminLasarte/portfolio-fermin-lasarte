...
//
// Las dos mitades conviven: una página que llegó con la cortina puesta también tiene
// que poder salir con ella. Antes la llegada cortaba el efecto con un `return` y en esa
// página los enlaces quedaban sin cortina, que es lo que pasaba al volver de Trayectoria
// a su sección (2026-09-21).
//
...
export default function Curtain() {
  useEffect(() => {
    ...
    // ---------- Llegada ----------
    // El script del tema ya dejó la pantalla tapada (el atributo `d` que trae el HTML
    // es justo esa posición, así que no hay nada que escribir antes de pintar).
    if (root.classList.contains("curtain-hold")) {
      let cortar = null;
      // Un frame para que la pista ya esté en su sección, y después la espera con el
      // nombre del destino a la vista antes de destapar.
      const raf = requestAnimationFrame(() => {
        ...
        limpiar.push(() => clearTimeout(espera));
      });
      ...
      }, CURTAIN_MS.hold + CURTAIN_MS.out + 400);
      limpiar.push(() => {
    ...
    // ---------- Salida ----------
    if (curtainSupported()) {
      // Solo los enlaces marcados con data-curtain, que son los que cambian de página
      // (las páginas propias, el 404, los del nav y el botón de idioma). El valor es el
      // nombre del destino, ya traducido por quien renderiza el enlace.
      let yendo = false;
      let cortar = null;
      let red = 0;
      ...
      const onClick = (e) => {
        ...
        // Apenas termina de tapar se va: la espera con el nombre a la vista la pone la
        // página de destino, que muestra el mismo texto. Como es el mismo, se lee
        // continuo aunque el documento cambie en el medio, y el recorrido no se alarga
        // dos veces.
      ...
      // Al volver con el botón "atrás", la página puede venir del bfcache con la
      // cortina todavía puesta: se saca y se vuelve a habilitar la salida.
      const onShow = () => {
        yendo = false;
        cortar?.();
        clearTimeout(red);
