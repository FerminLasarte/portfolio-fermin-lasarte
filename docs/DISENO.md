...
- **Fecha:** 2026-09-14
- **Estado:** lo aprobé el 2026-09-14 y lo implementé en la rama `fase-3-rediseno`, un commit por paso (sección 10). Esa rama se mergeó a `main` el 2026-09-15 (`006bdf0`) y se borró: el diseño está en producción. Donde la implementación se apartó del brief, este documento ya lo dice. Se puso al día con el código después de la re-auditoría (2026-09-15, R-M37), incluidos los cambios de la limpieza (R-M28 a R-M36).
- **Cambio posterior (2026-09-22):** rehice Proyectos (7.3) y le di a cada proyecto su página (7.12). Lo que cambió está escrito en esas dos secciones, en la tabla de movimiento (8) y en la franja (7.1); el resto del documento sigue valiendo.
- **Fuentes, en orden de prioridad:** los "Criterios del rediseño" de la auditoría que hice antes del rediseño; [douglus.site](https://douglus.site) (navegación horizontal y sus animaciones); las skills `design-taste-frontend`, `impeccable` y `emil-design-eng`. Si dos fuentes chocan, manda la de más arriba.
- **Códigos entre paréntesis:** los que aparecen en el texto (C5, I12, M14, R-M32...) son puntos de esa auditoría y de la re-auditoría que hice después. Son documentos de trabajo y no están en el repo.
...
| Proyectos | Capturas en marcos de escritorio | Un índice con la pantalla de cada app al lado; en la página de cada proyecto, las pantallas acompañan la lectura, sin caja de color |
...
El cursor propio y la foto arrastrable los sumé el 2026-09-14. La skill `design-taste-frontend` desaconseja los cursores propios, pero douglus.site va antes que las skills y mi criterio va antes que todo. Se rehacen sin los problemas de M7 y con las condiciones de 7.8 y 7.9.
...
- Las capturas de los proyectos van sobre el fondo de la página, sin caja de color (desde el 2026-09-23). Ya no hay colores de marca en los datos (`media.plate` se fue con la placa).
...
- **Los enlaces de afuera** (las tiendas, el código, la web; los devolví el 2026-09-23: se habían perdido con las tarjetas): en píldoras chicas (`.btn--sm`, con el relleno y el imán de siempre), **en cada fila**, al lado del enlace a la página y no adentro (un enlace no puede tener otro adentro). En la lista, debajo de la meta, alineados con el nombre. Con la vista previa aparecen en la fila activa, **en el lugar de la meta**, a la izquierda de la flecha: se llega a ellos moviendo el mouse de costado, sin cruzar otras filas (en la vista previa, a la derecha, había que bajar en diagonal y cada fila que se cruzaba cambiaba el proyecto). La meta pasa a la vista previa, arriba de la solución. En las otras filas quedan transparentes y sin puntero, pero se leen y se enfocan: el foco en uno activa su fila (`WorksHover`). Si la lista mide menos de 42rem (una consulta de contenedor), solo el icono, y el nombre queda para los lectores de pantalla.
...
- **Sin cortes entre paneles** (2026-09-14): los paneles no llevan bordes que los separen, ni en horizontal ni en vertical, y el pie tampoco. Los separa el aire (el margen lateral de cada panel).

### 7.4 Trayectoria

Rediseñada el 2026-09-15: elegí la propuesta "A · Etapas" del lienzo de propuestas. La anterior (un párrafo largo, un eje de años con barras y cuatro fichas) repetía las mismas fechas tres veces y no se entendía. En la home es una **vista previa**; el detalle completo va en su propia página (7.12).
...
Lo rediseñé el 2026-09-14 como el final de douglus (antes era un panel lleno de cobalto con el email en tamaño Mega y filas con tachado).
...
- Píldoras grandes (`.btn--night.btn--lg`, en caja normal): "Email" (`mailto:`), WhatsApp, LinkedIn y GitHub. A la derecha, "Volver al inicio" con una flecha larga: un ancla a `#sobre-mi` que pasa por el scroll suave (7.10). Los cuatro nombres son iguales en los dos idiomas, así que no pasan por el diccionario. Antes había también una píldora con el email escrito, otra con el teléfono como `tel:` y un botón "Copiar email" (copiaba al portapapeles y decía "Copiado" 2 s): los saqué el 2026-09-21 para dejar cuatro destinos y nada más.
...
- Queda donde frena, como en douglus (lo decidí el 2026-09-14), aunque tape el nombre, el párrafo o los botones: quien la movió la puede volver a mover.
...
Lo sumé el 2026-09-14. Hasta entonces el diseño decía que el scroll no se suavizaba.
...
- **Tiempos:** el contorno se dibuja de 100 a 900ms. Las pantallas arrancan en `--pl-screens` (950ms) y cambian cada `--pl-screen` (750ms), con un fundido cruzado de 100ms entre una y otra; cada una se arma en 500ms (sus piezas suben de a una, escalonadas en 200ms) y queda completa y quieta unos 250ms antes de irse. La pantalla se pinta en `--pl-paper` (3,15 s), cuando la última ya se vio; el zoom va de `--pl-zoom` (3,4 s) a 4,05 s, y arranca y termina despacio; el velo se va en `--pl-out` (3,95 s), en 250ms, y ahí entra el nombre del hero. En total, unos 4,2 s (el de douglus que tenía antes, 5,3 s).
- **Por qué esos tiempos:** la primera versión (2026-09-24) duraba 2,1 s, con cada pantalla 260ms y cortes secos, y la sentí rápida y brusca: no llegaba a verse ninguna pantalla. Lo alargué ese mismo día lo justo para que cada pantalla se vea armarse y se vea completa.
- **Solo CSS:** la secuencia son `@keyframes` con retrasos (`styles/preloader.css`), así que termina sola aunque el JS falle. Al final queda con `visibility: hidden` y deja de recibir clics. Como red de seguridad (por ejemplo, si el navegador no corre las animaciones), el script del tema saca `html.pl` a los 7 s.
...
- **Se puede saltear** (R-M3): la primera tecla, clic o giro de la rueda le pone `html.pl-skip`, que desvanece el velo en 250ms; el nombre del hero entra enseguida, con su animación de siempre. A los 3,4 s, con el zoom, el velo ya se está yendo solo y los listeners se sacan.
...
- **Costo:** unos 4 s de espera la primera vez (se puede saltear).

### 7.12 Páginas propias

Las sumé el 2026-09-15: Trayectoria y Habilidades son vistas previas en la home, y cada una tiene su página con más información, como las de douglus ("About", "Works").
El 2026-09-22 se sumó **Proyectos**, que además es la única con páginas hijas: una por proyecto.
- **Rutas:** en el idioma de cada versión: `/proyectos` y `/en/projects`, `/trayectoria` y `/en/experience`, `/habilidades` y `/en/skills`. Viven en `app/[lang]/[page]`, con `generateStaticParams` por idioma y `dynamicParams = false` (cualquier otra ruta da el 404 global). `lib/pages.mjs` tiene las rutas en un solo lugar (sin alias, porque también lo lee `next.config`, que reescribe la ruta española sin prefijo y redirige la que viene con `/es`); `pagePath()` y `pageUrl()` de `lib/i18n.js` arman los enlaces.
- **Página de cada proyecto:** `/proyectos/<id>` y `/en/projects/<id>`, en `app/[lang]/[page]/[slug]`. El slug es el `id` del proyecto y **es el mismo en los dos idiomas**, que es lo que deja que el botón de idioma se quede en el proyecto (`useSelectedLayoutSegments` da `["proyectos", "travelpic"]` y el nav le pega el slug al otro idioma). Cambiar un `id` rompe una URL publicada. Las reescrituras del español cubren la ruta y lo que cuelga de ella (`/proyectos/:slug`).
...
- **Proyectos (`/proyectos`):** el título, una frase y el índice de la home (7.3), el mismo componente (`ProjectIndex`): una sola definición de cómo se resume un proyecto. Existe sobre todo para que las páginas de proyecto tengan madre y las migas de pan cierren (Inicio › Proyectos › TravelPic).
- **Proyecto (`/proyectos/<id>`):** el número y la vuelta a la lista, el título en Display, el problema y la solución, el estado, las plataformas y los años (que salen de Trayectoria cuando el proyecto también es una etapa: no se inventan los que no están), la historia (las pantallas y los capítulos, 7.13), las tecnologías con los enlaces de afuera, el anterior y el siguiente en círculo, y "Volver al inicio", que vuelve a la fila de ese proyecto y no al principio de la sección. La meta y los enlaces son los mismos componentes que usa el índice (`ProjectMeta`, `ProjectLinks`).
- **Los bloques largos son opcionales:** `context`, `build`, `decisions` y `result` se muestran **solo si el proyecto tiene ese texto** (`projects.<id>.context` y compañía). Desde el 2026-09-23 los tienen los seis proyectos; uno nuevo sale con el resumen, las pantallas, las tecnologías y los enlaces hasta que tenga los suyos, y se llena sin tocar código. Una clave que no está en ninguno de los dos idiomas no rompe el control de paridad de `scripts/check.mjs`. Cómo se recorre la página está en 7.13.
- **Desde la home:** el botón "Ver trayectoria completa", al pie de la columna del título de Trayectoria; "Ver todas las habilidades", al lado del título de Habilidades; y "Ver el proyecto" en cada tarjeta de Proyectos, que es su botón relleno (7.3). En `/trayectoria`, "Ver el proyecto" de cada etapa dejó de apuntar a la tarjeta de la home y apunta a la página del proyecto, que es de lo que habla la etapa.

### 7.13 El recorrido de una página de proyecto

Lo decidí el 2026-09-22, al escribir la de DeporTurnos: que la página, su recorrido y sus animaciones se sientan parte del portfolio. Por eso **no inventa movimientos nuevos**: usa los de la home, en vertical.
- **Entradas:** las de los paneles (8, "Entrada de cada panel"), con el mismo código. La lógica salió de `TrackController` a `lib/reveal.js` y la usan la pista (un grupo por panel) y `components/PageReveal.jsx` (un grupo por bloque marcado con `data-reveal`: las pantallas, cada capítulo, las tecnologías y el pie). Los títulos de los capítulos suben en su máscara, las pantallas se destapan desde abajo y el resto sube con un fundido, 60ms detrás del anterior. Si se llega con la cortina, lo que ya se ve entra cuando la cortina se va, como en la home. El encabezado tiene su propia entrada al cargar.
- **Las pantallas** (`components/ProjectScreens.jsx`; el 2026-09-23 elegí la opción "C · La pantalla acompaña la lectura" entre tres bocetos: A, las capturas en la cabecera; B, una tira de pantallas con su descripción). Hasta ese día iban en una banda del color de la app arriba de los capítulos, con el teléfono en el medio, en abanico o en cascada, y con paralaje, y no me gustaba el fondo violeta con la captura en el medio. Ahora van **sin caja de color, sobre el fondo de la página**, en la columna izquierda de la historia (5fr; los capítulos, 7fr), que antes quedaba vacía debajo de cada título. Desde escritorio y con JS quedan **fijas mientras se lee** (`sticky`) y se ve una sola: la del capítulo que cruza una línea al 45% del alto de la ventana (`components/ScreenSync.jsx`, con un `IntersectionObserver`). El capítulo n muestra la captura n (la principal con Contexto, después las de `media.gallery` en orden); los que sobran, y Tecnologías, la última. Cambian con un fundido (`--dur-ui`), y las otras quedan transparentes en la misma celda, así se siguen leyendo con su descripción. Abajo, unos puntos dicen cuál de cuántas (decorativos). El teléfono toma el alto de la pantalla menos el nav (hasta 44rem) y la ventana, el ancho de la columna. Sin JS, todas, una debajo de otra, al lado del texto. En celular y tablet van antes de los capítulos: dos o tres teléfonos en abanico (el principal adelante, los otros detrás, al 86%, corridos un 62% y girados 6°; el alto sale del ancho para que no toquen los bordes), dos o tres ventanas en cascada, y de más de tres (DeporTurnos, con cinco), una fila que se desliza de costado, de borde a borde, con la siguiente asomando; se enfoca con Tab y lleva su nombre ("Pantallas de DeporTurnos").
- **Apps de escritorio (`media.type: "window"`, Vault):** la captura va en una ventana de macOS, con su barra y los tres botones de colores (la única seña que se entiende sin explicar). Todo se mide desde el ancho de la ventana (`--win-w`), así es la misma en el índice y en la página. Con `imageDark`, cada captura tiene su versión oscura y se muestra la del tema del sitio; la otra queda con `display: none`, que no se lee ni, siendo lazy, se descarga.
- **La historia:** los bloques son capítulos numerados (01 Contexto, 02 Qué construí...): el número chico, el título y el texto, **uno arriba del otro y al ras del mismo borde**. Así no hay dos columnas que alinear: con el título a la izquierda y el texto a la derecha, aunque compartieran la línea de base, el título asomaba unos 20px por arriba de la primera línea y el número lo corría 40px del borde de todo lo demás. En Resultado, las cifras van entre el título y el texto. "Tecnologías" cierra la historia en la misma columna, con una línea y más aire. Sin capturas (chatbot-ai, el Compilador), los capítulos van solos, al ras del borde de la página, como la cabecera. Una línea los une por la izquierda, con un punto del acento en el número de cada uno, y **se dibuja mientras se lee**: desde que los capítulos llegan al 60% de la ventana hasta que su final pasa por el mismo lugar. Va en el margen de la página o, con pantallas, en el aire entre las dos columnas. Es la línea con puntos de Trayectoria en la home.
...
- **Sin soporte, sin JS o con reduce motion:** la línea entera y el siguiente proyecto lleno; sin JS, las pantallas una debajo de otra. En colores forzados, la línea y los puntos van en `CanvasText` y el siguiente proyecto, lleno en el color de los enlaces.
...
| Preloader | Primera carga de la sesión | `stroke-dashoffset` del contorno, `opacity` de las pantallas, `opacity` y `translate` Y de sus piezas, `scale` del teléfono, `opacity` del velo | Unos 4,2 s en total (7.11); con una tecla, un clic o la rueda, el velo se desvanece en 250ms | No existe |
...
| Pantallas de una página de proyecto (7.13) | El capítulo que se lee cruza el 45% del alto (`IntersectionObserver`) | `opacity` de cada captura; ancho y color del punto activo | `--dur-ui` `--ease-out` | Fundido; el punto, sin transición |
| Línea de la historia (7.13) | Scroll | `scale` Y del `::before` de `.story__chapters` | Lineal, `view()` de los capítulos: `cover 40vh` a `cover calc(100% − 60vh)` | Entera |
...
| Nombre del hero, letra por letra | Carga (con el preloader, cuando se va el velo, a los 3,95 s) | `translate` Y desde 105%, dentro de una máscara | 700ms `--ease-expo`, 28ms entre letras | Sin animación |
...
| Cortina entre páginas | Cualquier enlace que cambie de página: las páginas propias (7.12), los de sección del nav y la marca desde una página propia, el botón de idioma y el 404 | La de douglus, con sus mismas posiciones: un `<path>` de un SVG `viewBox="0 0 100 100"` con `preserveAspectRatio="none"` que sube desde abajo con el borde curvo hasta tapar (`M 0 100 V 100 Q 50 100 100 100 V 100 z` → `V 50 Q 50 0 100 50` → `V 0 Q 50 0 100 0`), muestra el nombre del destino subiendo en su máscara con una línea que se dibuja, y del otro lado se retira hacia arriba (`M 0 0 V 100 Q 50 100 100 100 V 0 z` → `V 50 Q 50 100 100 50` → `V 0`). El borde lo mueve `components/Curtain.jsx` escribiendo el atributo `d` con un `requestAnimationFrame`, como hace GSAP en douglus | Tapa en 1 s (700ms hasta la curva y 300ms hasta el techo). El nombre entra a 500ms y la línea a 700ms, mientras todavía sube: así ya están cuando termina de tapar. Del otro lado la página llega tapada y espera 500ms con el nombre a la vista antes de retirarse en 1,05 s. La espera va solo de ese lado: el nombre es el mismo texto en las dos páginas, así que se lee continuo aunque el documento cambie en el medio | Sin cortina: la navegación es la de siempre |
...
- **El nombre va encima del path.** `.curtain__stage` es `position: relative` con `z-index`: el SVG es `absolute` y, por orden de pintado, lo posicionado va sobre lo que no lo está, así que sin eso el path negro tapaba el nombre. Hasta que el movimiento estuvo en CSS no se notaba, porque la animación del path lo subía a su propia capa (2026-09-21).
...
- **Por qué el movimiento va por JS y no por CSS.** La primera versión animaba la propiedad CSS `d` con `@keyframes`, dentro de un `@supports (d: path(...))`. Funciona en Chrome y en Firefox, pero **Safari no soporta esa propiedad en ninguna versión** (caniuse, hasta la 27.2), así que ahí el `@supports` daba falso, `.curtain` nunca salía de `display: none` y, como `Curtain.jsx` preguntaba lo mismo con `CSS.supports`, tampoco frenaba el clic: la cortina no se veía y el parpadeo del hero volvía tal cual. Lo encontré en producción el 2026-09-21. El atributo `d` sí se puede escribir en todos lados, así que ahora lo interpola `lib/curtain.js` (`mezclar` y `animar`) y queda un solo camino de código, sin `@supports`. En CSS quedan el nombre y la línea, que van con `translate`, `opacity` y `scale`.
- **Nunca queda tapada.** Si los frames no corren (la pestaña en segundo plano los pausa) hay dos respaldos: el timeout de `CURTAIN_MS` en `Curtain.jsx` y, si todo lo demás falla, los 4 s de `CURTAIN_FAILSAFE_MS` en el script del tema. `scripts/check.mjs` controla que `CURTAIN_MS` y los tokens `--dur-curtain-*` no se separen, y que `themeInitScript` parsee.
...
| D6 | Cursor propio y foto arrastrable | **Lo decidí el 2026-09-14:** se hacen como en douglus (7.8 y 7.9). `PremiumCursor` se reemplaza. Los números "01/" no se toman. El preloader tampoco se tomaba, pero lo sumé después (7.11) | Implementado |
| D7 | Firefox | Respaldo de JS (6.4), en vez de dejarlo en vertical | Implementado, sin probar en Firefox: lo dejé afuera de la re-auditoría (N6) |
...
Después vinieron mis rondas de cambios (las fechas están en cada sección) y la re-auditoría de la Fase 4.
